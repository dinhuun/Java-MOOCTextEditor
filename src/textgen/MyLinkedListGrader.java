...
/** Change .next and .prev (private, no longer available) to .getNext() and .getPrev() as designed in MyLinkedList.java
 * delete import java.io.FileNotFoundException and import java.io.UnsupportedEncodingException;
 */
...
public class MyLinkedListGrader {
	...
	public String printListForwards(MyLinkedList<Integer> lst)
	{
		LLNode<Integer> curr;
                String ret = "";
		if (lst.head.getData() == null)
			curr = lst.head.getNext();
		else
			curr = lst.head;
	...
	public String printListBackwards(MyLinkedList<Integer> lst) {
		LLNode<Integer> curr;
                String ret = "";
		if (lst.tail.getData() == null)
			curr = lst.tail.getPrev();
		else
			curr = lst.tail;
		while (curr != null && curr.getData() != null)
		{
